...
export async function emitirPase(uid) {
  ...
  if (select) {
    try {
      ...
      const fbdb = (await import("./firebase-config.js?v=10.21")).db;
      const { getPath } = await import("./firebase-config.js?v=10.21");
...
export async function confirmarEmitirPase() {
  ...
  try {
    ...
    const fbdb = (await import("./firebase-config.js?v=10.21")).db;
    const { appId } = await import("./firebase-config.js?v=10.21");
