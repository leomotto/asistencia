...
import { db, getPath } from "./firebase-config.js?v=9.97";
import { showToast } from "./ui.js?v=9.97";
import { HORARIOS_DINAMICOS } from "./materias.js?v=9.97";
import { normalizeDateToISO, formatISOToDisplay, escaparHTML } from "./utils.js?v=9.97";
...
export async function emitirPase(uid) {
  ...
  if (select) {
    try {
      ...
      const fbdb = (await import("./firebase-config.js?v=9.97")).db;
      const { getPath } = await import("./firebase-config.js?v=9.97");
...
export async function confirmarEmitirPase() {
  ...
  try {
    ...
    const fbdb = (await import("./firebase-config.js?v=9.97")).db;
    const { appId } = await import("./firebase-config.js?v=9.97");
