...
import { showToast } from "./ui.js?v=10.93";
import { HORARIOS_DINAMICOS } from "./materias.js?v=10.93";
import { normalizeDateToISO, formatISOToDisplay, escaparHTML } from "./utils.js?v=10.93";
...
export async function abrirModalAlumnoConId(id) {
  let alumno = window.app.alumnosMatriculaCache.find(a => a.id === id);
  if (!alumno) {
    // Cache vacío (ej. se entró desde Auditoría sin haber pasado antes por Matrícula): leer directo.
    const snap = await getDoc(doc(db, getPath('estudiantes'), id));
    if (!snap.exists()) { showToast('No se encontró el estudiante.', 'error'); return; }
    alumno = { id: snap.id, ...snap.data() };
  }
...
export function abrirModalAlumno(alumno = null) {
  ...
  const btnReactivarFicha = document.getElementById('btnReactivarAlumnoFicha');
  if (btnReactivarFicha) {
    if (alumno && esAdmin && alumno.estado === 'BAJA') {
      btnReactivarFicha.classList.remove('hidden');
      btnReactivarFicha.onclick = () => window.app.reactivarAlumno(alumno.id);
    } else {
      btnReactivarFicha.classList.add('hidden');
      btnReactivarFicha.onclick = null;
    }
  }
  const btnPaseFicha = document.getElementById('btnEmitirPaseFicha');
  if (btnPaseFicha) {
    if (alumno && esAdmin) {
      btnPaseFicha.classList.remove('hidden');
      btnPaseFicha.onclick = () => window.app.emitirPase(alumno.id);
    } else {
      btnPaseFicha.classList.add('hidden');
      btnPaseFicha.onclick = null;
    }
  }
...
export async function guardarAlumnoMatricula() {
  ...
  // Arranca con TODO el historial existente (incluye divisiones previas cuyo checkbox ya no
  // está tildado en este formulario) para no perderlo al guardar; solo se pisan las materias
  // actualmente tildadas, más abajo.
  const materiasSeleccionadas = [], gruposMap = {};
  const inscripcionesMap = JSON.parse(JSON.stringify(alumnoActual?.inscripciones || {}));
...
export async function emitirPase(uid) {
  ...
  if (select) {
    try {
      ...
      const fbdb = (await import("./firebase-config.js?v=10.93")).db;
      const { getPath } = await import("./firebase-config.js?v=10.93");
...
export async function confirmarEmitirPase() {
  ...
  try {
    ...
    const fbdb = (await import("./firebase-config.js?v=10.93")).db;
    const { appId } = await import("./firebase-config.js?v=10.93");
