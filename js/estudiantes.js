...
import { showToast } from "./ui.js?v=10.01";
import { HORARIOS_DINAMICOS } from "./materias.js?v=10.01";
import { normalizeDateToISO, formatISOToDisplay, escaparHTML } from "./utils.js?v=10.01";
...
export async function emitirPase(uid) {
  ...
  if (select) {
    try {
      ...
      const fbdb = (await import("./firebase-config.js?v=10.01")).db;
      const { getPath } = await import("./firebase-config.js?v=10.01");
...
export async function confirmarEmitirPase() {
  ...
  try {
    ...
    const fbdb = (await import("./firebase-config.js?v=10.01")).db;
    const { appId } = await import("./firebase-config.js?v=10.01");
