...
import { HORARIOS_DINAMICOS } from "./materias.js?v=10.78";
import { normalizeDateToISO, formatISOToDisplay, escaparHTML } from "./utils.js?v=10.78";
...
export async function emitirPase(uid) {
  ...
  if (select) {
    try {
      ...
      const fbdb = (await import("./firebase-config.js?v=10.78")).db;
      const { getPath } = await import("./firebase-config.js?v=10.78");
...
export async function confirmarEmitirPase() {
  ...
  try {
    ...
    const fbdb = (await import("./firebase-config.js?v=10.78")).db;
    const { appId } = await import("./firebase-config.js?v=10.78");
